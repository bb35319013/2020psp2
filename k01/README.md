...
1.32　X(1)~X(n)までを2乗したものの平均値を求める

1.34 nが1の時分散が推定できないのでsuiteivarに0を代入
...
[comment #20200617]
- このファイルの最初の部分は自分の学籍番号と氏名にしてください.
- 出力結果をこのレポートに貼り付けてください. 
- 「母集団平均(推定値)」, 「母集団分散(推定値)」だけでなく, 「標本平均」と「標本分散」も出力してください。
  - `var_online関数`では, 「標本分散」を計算するようにして, 「母集団分散(推定値)」は`printf()`するときに計算すると良いと思います. 
- `double n`と`int N`はおそらく役割的に同じだと思うので`int N`に統一しましょう. 適宜`double型`などにキャストすると良いと思います. 
- 変数名`x`では何を表しているのかすぐには分からないため, 分かりやすい変数名にしましょう. 2乗したものの平均だと思うので, 例えば`square_ave`などにしてはどうでしょうか. 
- プロトタイプ宣言には`extern`を付けますが, 関数本体には付けません.  (l.42, l.47)  
また, 最後に`;`も要りません. (l.45, l.55)
- インデントが揃っていないところがあります. 揃えましょう. (while文内)  
